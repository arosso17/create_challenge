...
def step(nodes, rule):
    ...
    for i, node in enumerate(nodes):
        locality = ''.join(['1' if b else '0' for b in [node.left.on, node.on, node.right.on]])
        board.append(apply_rule(locality, rule))
    for i, node in enumerate(nodes):
        node.on = board[i]

def apply_rule(locality, rule):
    rule = format(rule, '08b')
    locality = 7 - int(locality, 2)
    return True if int(rule[locality]) else False
